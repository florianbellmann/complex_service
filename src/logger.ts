...
if (process.env.NODE_ENV === "development") {
  transports.push(
    new winston.transports.File({
      ...
      filename: `debug_${logDate.getFullYear()}-${String(
        `00${logDate.getMonth() + 1}`
      ).slice(-2)}-${String(`00${logDate.getDate()}`).slice(-2)}.log`,
  ...
  transports.push(
    new winston.transports.File({
      dirname: path.resolve(process.env.LOG_LOCATION || "logs"),
      filename: `log_${logDate.getFullYear()}-${String(
        `00${logDate.getMonth() + 1}`
      ).slice(-2)}-${String(`00${logDate.getDate()}`).slice(-2)}.log`,
    })
  );
...
transports.push(
  new winston.transports.File({
    dirname: path.resolve(process.env.LOG_LOCATION || "logs"),
    filename: `error_${logDate.getFullYear()}-${String(
      `00${logDate.getMonth() + 1}`
    ).slice(-2)}-${String(`00${logDate.getDate()}`).slice(-2)}.log`,
